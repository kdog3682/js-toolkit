...
class LineEditor {
    ...
    find(x, dir) {
        assertEqual(dir, 1, -1)
        const fn = isRegExp(x) ? (s) => x.test(s) : x
        if (dir == 1) {
    ...
    slice(a, b, mode) {
        const lines = this.lines.slice(a, b + 1)
        switch(mode) {
            case String:
                return lines.join('\n')
            default:
    ...
    insertBelow(...args) {
        const [index, content] = args.length == 1 ? [this.pos, args[0]] : args
        this.lines.splice(index + 1, 0, this.indent(content, index, 1))
        return this
    }
    insertAbove(...args) {
        const [index, content] = args.length == 1 ? [this.pos, args[0]] : args
        this.lines.splice(index, 0, this.indent(content, index, -1))
        return this
    }
    indent(content, index, dir) {
        const s = this.get(index)
        const ind = getIndent(s)
        const extraInd = getExtraIndent(s, dir)
        const computedInd = ind + extraInd
        return indent2(content, computedInd)
    }
    toString() {
        return this.lines.join('\n')
    }
    setPos(r) {
        const i = isNumber(r) ? r : this.find(r)
        if (i == null) {
            panic('could not find index based on regex: $r', r)
        }
        this.pos = i
        return this
    }
