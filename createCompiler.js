...
class ExtendableVisitor {
    constructor(visitors) {
        const create = (x) => [x.type, x.visit.bind(this)]
        this.visit = this.visit.bind(this)
        this.visitors = reduce(visitors, create)
    }
    ...
    visitChildren(node, ...args) {
        ...
        const children = node.children.map(visitor).filter(isDefined)
        return children
    }
    visit(node, ...args) {
        if (node == null) {
            panic('the node is null')
        }
        const fn = this.visitors[node.type]
        if (fn) {
            return fn(node, ...args)
        }
        if (isString(node)) {
        ...
        // throw new Error('aa')
        // throw 'asdf'
        panic('$type is not in the visitorsRef. Additionally, the value for node.value is not a string. This node ... is not visitable. Consider writing a visit function for it: $self', node)
    }
}
class ExtendableParser {
    ...
    parseSingleton(type) {
        ...
        // Word and Symbol do not have their own parse functions
        // Therefore, we just eat.
    }
...
function createCompiler(items) {
    ...
    return function compile(s, mode) {
        const tokens = tok.tokenize(s)
        const ast = parser.parse(tokens)
        return mode == 'ast' ? ast : visitor.visit(ast)
    }
}

function createVisitor(items, mode) {
    const getVisitor = (spec) => {
        return spec.visit
    }
    if (mode == 'json') {
        items = items.map(editf('visit', 'jsonVisit'))
    }
    const visitors = filter(items, getVisitor)
    const visitor = new ExtendableVisitor(visitors)
    return visitor
}
