...
function chalk(...args) {
    const fn = isNode() ? nodeChalk : browserChalk
    return fn(...args)
}

function chalkf(color, topBold) {
    const fn = isNode() ? nodeChalk : browserChalk
    return function lambda(s, bold) {
        return fn(s, color, isDefined(bold) ? bold : topBold)
    }
}
...
function nodeChalk(arg, color, bold) {
    const colorValue = variables.terminalColors[color]
    const reset = variables.terminalColors.RESET
    const boldValue = bold ? variables.terminalColors.BOLD : ''
    return colorValue + boldValue + stringify(arg) + reset
}

const black = chalkf("BLACK", true)
const red = chalkf("RED")
const green = chalkf("GREEN")
const blue = chalkf("BLUE")

function grid(...args) {
    for (let i = 0; i < args.length; i+= 2) {
        const [a, b] = args.slice(i, i + 2)
        const first = blue(a, true)
        const second = blue(b)
...
function printAnsi(s) {
    // Print the ANSI escape codes table
    for (const category in ansi) {
      console.log(`\n${category}:`);
      for (const style in ansi[category]) {
        console.log(`${style}: ${ansi[category][style]}Sample Text${ansi["style"]["reset"]}`);
      }
    }
...
function bold(s) {
    return ansi.style.bold + s + ansi.style.reset
}
function hr(n = 60) {
    return ansi.style.underline + ' '.repeat(n) + ansi.style.reset
}
...
function numbered(items, display) {
    ...
    items.forEach((item, i) => {
        const value = display ? display(item) : item
        const delimiter = (i + 1) + "."
        const front = bold(delimiter)
...
function announce(x, y) {
    ...
    const statement = tail(caller.file) + ':' + caller.line
    return console.log(bold(statement), blue(x), y)
}
// numbered('aaa'.split(''))
