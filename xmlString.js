...
function builder(key, value) {
    return ' ' + attrCase(key) + ' = ' + doubleQuote(value)
}

function rts(o, da, db) {
    let s = ''
    for (const [k, v] of entries(o)) {
        s += dashCase(k) + da + v + db
    }
    return s
}
function buildAttrs(attrs) {
    let s = ''
    if (attrs.style) {
        let t = rts(attrs.style, ':', ';')
        s += builder('style', t)
    }
    if (attrs.classes) {
        s += builder('class', attrs.classes.join(' '))
    }
    if (attrs.attributes) {
...
function xmlString(data, children) {
    const { tag, value, attrs, newlines } = data

    const attrString = buildAttrs(attrs)
    const heading = tagRef[tag] || tag || 'div'
    const body = join(children) || value

    const htmlBody = hasNewline(body)
        ? newlineIndent(body)
        : body
...
function buildAttrString(data) {
    ...
    const className = classes ? classes.join(" ").trim() : null
    ...
    return startingSpace(aggregate)
}
