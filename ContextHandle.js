...
class ContextHandle {
    ...
    get input() {
        return this.vue.input
    }
    set input(input) {
        this.vue.input = input
    }

    setup(c) {
        this.name = c.name
        const handle = c.handle || defaultHandle
        this.handle = (handle).bind(this)
        this.onStart = c.onStart ? c.onStart.bind(this) : noop
        this.onEnd = c.onEnd ? c.onEnd.bind(this) : noop
        this.mounted = c.mounted ? c.mounted.bind(this) : noop
        this.handlers = walk(c.handlers, (v) => v.bind(this.vue))
    }
}
...
class HandleControl {
    constructor(vue, { options = {}, contexts = [] } = {}) {
        ...
        const init = (acc, context) => {
            acc[context.name] = new ContextHandle(context, vue)
            return acc
        }

        this.contexts = contexts.reduce(init, {})
        this.contextList = Object.keys(this.contexts)
        const key = this.options.key || contexts[0].name
        console.show("initial context key", { key })
        assert(key)
        this.setContext(key)
    }
    setContext(name, props) {
        if (this.context) {
            console.show("setting previous context to", this.context.name)
        ...
        console.show("setting new context to", name)
        this.vue.input = this.context.cache.get("input") || ""
        this.context.onStart(props)

        const contextHandle = this.context.handle
        const listener = this.setListener(contextHandle)
    ...
    setListener(contextHandle) {
        ...
        const listener = (e) => {
            ...
            preventDefault(e)
            const [key, primaryType, secondaryType] = getKeyAndType(e)
            const inputState = {
                key,
                primaryType,
                secondaryType,
...
function defaultHandle(inputState, state) {
    const { key, primaryType, secondaryType, input } = inputState
    const before = this.handlers.before || state.handlers.before
    const after = this.handlers.after || state.handlers.after
    const fn =
        this.handlers[primaryType] ||
        this.handlers[secondaryType] ||
        state.handlers[primaryType]
    const length = key.length
    const currentInput = length == 1 ? input + key : input
    if (before) {
        before(currentInput)
    }

    const result = fn ? fn(key, currentInput) : null
    // console.show({result, input: this.vue.input, input, length})
    if (result === false) {
    } else if (this.vue.input.length != input.length) {
    ...
    if (after) {
        after(this.vue.input)
    }
}
class WindowControl extends HandleControl {
    constructor(vue) {
        const base = vue.$options.contextHandles || [
            {name: 'default',
             handlers: vue.$options.contextHandle}]
        const mapper = ([k, v]) => {
            return { name: k, handlers: v }
        }
        const contexts = isObject(base) ? entries(base).map(mapper) : base
        super(vue, { contexts })
    }
}
