...
class FireStore {
    ...
    async getAll() {
        const snaps = await getDocs(this.ref);
        return snaps.map(getDocData)
    }
    getDoc(x) {
        if (isString(x)) {
            return doc(this.db, this.collectionId, x)
        }
        if (isObjectWithKey(x, 'id')) {
            return doc(this.db, this.collectionId, x.id)
        }
        panic('could not retrieve a doc for $1', x)
    }
    async get(docId) {
        const docSnap = await getDoc(this.getDoc(docId))
        if (docSnap.exists()) {
    ...
    async create(...args) {
        const [name, v] = args.length== 1 ? [null, args[0]] : args
        if (empty(v)) {
            return 
        }
        typeAssertion(v, Object)

        if (name) {
            const docRef = this.getDoc(name)
            await setDoc(docRef, v)
            return docRef
        } else {
            const col = collection(this.db, this.collectionId)
            const docRef = await addDoc(col, v)
            return docRef
        }
    }

    async increment(docId, key, n = 1) {
        await updateDoc(this.getDoc(docId), {
            [key]: increment(n),
            lastUpdated: serverTimestamp(),
        })
    }

    async update(docId, v) {
        if (empty(v)) {
            return 
        }
        if (v.serverTimestamp) {
            v.serverTimestamp = serverTimestamp()
        }
        await updateDoc(this.getDoc(docId), v)
    }

    async find(obj, num = 1) {
        const hits = Object.entries(obj).map(([a,b]) => {
            return where(a, '==', b)
        })
        const snaps = await getDocs(query(this.ref, ...hits, limit(num)))
        const datas = snaps.docs.map(getDocData)
        if (num == 1) {
            return datas[0]
        }
        return datas
    ...
    async findall(obj) {
        ...
        const snaps = await getDocs(query(this.ref, ...hits))
        return snaps.docs.map(getDocData)
    }
    async delete(doc) {
