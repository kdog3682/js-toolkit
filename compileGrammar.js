...
const GrammarSpecs = [
    { type: "Word", txype: "WORD", regex: /^[a-zA-Z]\w*/, kind: "identifier",

        visit(node) {
            return node.value.toUpperCase()
        }
    },
    { type: "Space", txype: "SPACE", regex: /^\s+/, kind: "whitespace", skip: true, },
    { type: "OpenParenthesis", tyxpe: "OPEN_PARENTHESIS", regex: /^\(/, kind: "delimiter" },
    { type: "CloseParenthesis", tyxpe: "CLOSE_PARENTHESIS", regex: /^\)/, kind: "delimiter" },
    { type: "Symbol", tyxpe: "SYMBOL", regex: /^[+?*]/, kind: "operator", description: "One or more repetitions",},
    { 
        type: "Group", 
        parse() {
            if (this.peekType('OpenParenthesis')) {
                this.eat('OpenParenthesis')
                const expr = this.parseMultiple('Element', {
                    parentType: 'Group'})
                this.eat('CloseParenthesis')
                return expr
            }
        },
        visit(node) {
    ...
    },
    {
        type: 'Element',
        parse() {
            const atom = this.parseSingleton('Word')
                || this.parseSingleton('Group')

            if (atom == null) {
                return 
            }
            const symbol = this.parseSingleton('Symbol') || null
        ...
        visit(node) {
            if (node.value) {
                let base = this.visit(node.value.atom)
                if (node.value.symbol) {
                    base += this.visit(node.value.symbol)
                }
                return base
            }
            if (node.children) {
