...
function solve(s, ref, mode) {
    ...
    if (isInequality(s)) {
        todo()
        const key = match(s, /[a-z]/)
        return text(nerd.solveFor(key)[0])
    }

    if (isEquation(s)) {
        const key = match(s, /[a-z]/)
        return text(nerd.solveFor(key)[0], mode)
    } 

    return text(nerd, mode)
}
...
function expand(s) {
    return text(nerdamer.expand(s))
}
function text(x, mode = 'fractions') {
    /* mode: fractions | recurring | decimals */
    if (mode == Number) {
        return Number(x.text('decimals'))
    }
    const value = x.text(mode)
    return value
}

function simplify(s) {
    const simplified = nerdamer.simplify(s)
    return text(simplified)
}


// console.log(solve('1/2 + 2/3 - 1/66'))
// console.log(solve('1/2 + 2/3 - 1/6'))
// console.log(evaluate('1/2 + 2/3 - 1/6'))
// console.log(solve('1/2 + 2/3 - 1/6 = x - 3'))
// console.log(solve('1/2 + 2/3 - 1/6 < x - 3'))
// console.log(solve('2 < 3'))
// console.log(solve('40 * x/100 = 60'))
// console.log(solve('x*30% = 20'))
// console.log(solve('5! = x - 1'))
// console.log(expand(simplify('2x + 2y')))

function equationReplace(s, ref) {

    const replacer = (letters, offset, o) => {
        let prefix = ' * '
        for (let i = 1;; i++) {
            const ch = o[offset - i]
            if (ch == ' ') {
        ...
        const value = ref[letters] || letters
        return prefix + value
    }
...
function abc(ref) {
    let s = `(2 * radius * n) + (n - 1) * spacing = width`
    let count = 0
    function runner(ref) {
        const val = solve(s, ref, Number)
        if (isInteger(val)) {
            return val
        }
        if (++count == 3) {
            ...
            panic('needs to hit')
            return runner(adjust(ref, -1))
            // need smaller padding or smaller radius
            // currently, the spacing is too big
        }
        else {
            panic('needs to hit')
            return runner(adjust(ref, +1))
            // need bigger padding or bigger radius
        }
    }
    function adjust(ref, dir) {
        ref.spacing += dir
        return ref
    }
    return runner(copy(ref))
}

function googa(width) {
    const ref = {
        radius: 10,
        spacing: 10,
        width,
    ...
    const num = abc(ref)
    return circles(num, ref)
}
function circles(num, {width, spacing = 5, radius = 5} = {}) {
    const store = []
    let pos = 0
    for (let i = 0; i < num; i++) {
        pos += radius
